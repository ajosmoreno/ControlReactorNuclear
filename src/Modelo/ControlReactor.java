...
public class ControlReactor {
     ...
     * @throws IOException 
     */
    public void subirBarra(int i, int j) throws IOException, IllegalArgumentException{
        if(i <= devolverNumeroBarras() || j <= devolverNumeroBarras()){
            if(!log.exists()) log.createNewFile();
            ...
            throw new IllegalArgumentException("La barra introducida no existe.");
        }
    }
     ...
     * @throws IOException 
     */
    public void bajarBarra(int i, int j) throws IOException, IllegalArgumentException{
        if(i <= devolverNumeroBarras() || j <= devolverNumeroBarras()){
            if(!log.exists()) log.createNewFile();
            estadoBarras[i][j] = true;
            PrintWriter pw = new PrintWriter(new FileWriter(log, true));
